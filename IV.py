...
def IV():
    ...
    # get input
    while True:
        call_price_ans = input(" Call Midpoint: ")
        ...
        if not check:
            continue
        days_to_exp_ans = input(" Days to expiration: ")
        ...
        if not check:
            continue
        sigma_guess_ans = input(" Guess Sigma: ")
    ...
    while error > tolerance:
        ...
        call_expect = (stock_price * norm.cdf(d1) -
                       (strike_price * np.exp(-risk_free_rate * days_to_exp)) * norm.cdf(d2)) - call_price
        ...
        print(f"Call Price Estimate: {call_price} : {call_expect + call_price}")
        print("")
    error_count = []
